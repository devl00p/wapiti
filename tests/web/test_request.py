...
@pytest.mark.parametrize(
    "request_, response_, expected_output",
    [
        (
    ...
    user-agent: Wapiti'''
        ...
        (
            Request("http://perdu.com/", headers={"User-Agent": "Original", "X-Custom": "Keep"}),
            WapitiResponse(
                httpx.Response(
                    200,
                    request=httpx.Request(
                        "GET", "http://perdu.com/", headers={"User-Agent": "Sent", "X-New": "New"}
                    )
                )
            ),
            '''GET / HTTP/1.1
...
def test_http_repr(request_, response_, expected_output):
    output = request_.http_repr(response=response_).strip()
...
def test_curl_repr(request_, expected_output):
    assert request_.curl_repr == expected_output
